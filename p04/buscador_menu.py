...
def buscar_por_capitulos(soup, consulta):
    ...
    for i in range(len(encabezados)):
        ...
        if consulta.lower() in contenido_capitulo.lower():
            resultados.append(
                f"Capítulo/Sección: {titulo_capitulo}\n{contenido_capitulo[:200]}... [Continúa]"
            )
...
def buscar_contexto_palabras(texto, consulta, margen=30):
    """Busca la palabra y devuelve N palabras antes y N palabras después."""
    # Dividimos todo el texto en una lista de palabras (nuestro "índice" básico)
    palabras = texto.split()
    resultados = []
    consulta_lower = consulta.lower()

    for i, palabra in enumerate(palabras):
        # Limpiamos signos de puntuación pegados a la palabra para comparar bien
        palabra_limpia = palabra.strip(".,;:\"'()!?").lower()

        if palabra_limpia == consulta_lower:
            # Calculamos los índices con cuidado de no salirnos de los límites de la lista
...
def main():
    ...
    while True:
        ...
        for idx, res in enumerate(resultados[:5]):
            print(f"\nCoincidencia {idx + 1}:")
            print(res)
