...
class BuscadorPLNApp(App):
    ...
    def ejecutar_busqueda(self) -> None:
        ...
        if not ruta_archivo.exists():
            log.write(
                f"[bold red]Error:[/bold red] No se encontró el archivo '{ruta_archivo}'."
            )
            return
        ...
        log.write(
            "[bold cyan]2. Extrayendo texto limpio con BeautifulSoup4...[/bold cyan]"
        )
        ...
        log.write(
            f"[bold cyan]3. Procesando {len(lineas)} líneas con Joblib (backend='threading')...[/bold cyan]\n"
        )
        ...
        log.write(
            f"La palabra [bold yellow]'{palabra_objetivo}'[/bold yellow] aparece en [bold red]{len(coincidencias)}[/bold red] líneas distintas.\n"
        )
        log.write("-" * 50)
