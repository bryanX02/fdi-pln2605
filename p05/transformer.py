...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        # Subimos a la dimensión oculta, activamos, bajamos y aplicamos dropout
        x = self.up(x)
        x = self.act(x)
        x = self.down(x)
        x = self.dropout(x)
...
class Block(nn.Module):
    ...
    def forward(self, x, causal=True):
        ...
        # Primero atención: normalizamos, aplicamos atención, y sumamos (residual)
        x = x + self.attn(self.norm1(x), causal=causal)
        # Luego feed-forward: normalizamos, aplicamos FF, y sumamos (residual)
...
class Transformer(nn.Module):
    ...
    def forward(self, idx, causal=True):
        ...
        # Sumamos embeddings de token y posición, y aplicamos dropout
        x = self.drop(self.tok_emb(idx) + pos)

        # Pasamos por cada bloque del transformer
